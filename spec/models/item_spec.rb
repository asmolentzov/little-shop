  ...
  describe 'instance methods' do
    it '.avg_fulfill_time' do
      ...
      order_item_1 = OrderItem.create(order_id: order_1.id, item_id: item_1.id,
                      quantity: 1, order_price: item_1.current_price, fulfilled: true, created_at: 2.days.ago)
      order_item_2 = OrderItem.create(order_id: order_1.id, item_id: item_1.id,
                      quantity: 1, order_price: item_1.current_price, fulfilled: true, created_at: 3.days.ago)

      expect(item_1.avg_fulfill_time).to eq(2)
    end
  end
end
